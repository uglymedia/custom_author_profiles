<?php
/**
 * @version um_cap_admin.class.php Jul 19, 2018 14:15:58 Nicholas Moller
 *
...
class um_cap_admin {
	...
	public function __construct(){
		...
		$this->field_prefix = um_cap::$field_prefix;

		// outside the admin menus scope for redirection purposes.
		add_action('init', array($this, 'delete_field'));

		add_action('admin_menu', array($this, 'add_menus'));
		add_filter('plugin_action_links_' . UM_CAP_PLUGIN_NAME, array($this, 'add_plugin_action_links'));
	...
	public function admin_route(){
		...
		if(isset($_GET['page'])){
			switch($_GET['page']){
				case 'um_cap_settings':
					$this->um_cap_settings();
					break;
				case 'um_cap_fields':
					if(isset($_GET['func'])){
	 ...
	 *
	 */
	private function um_cap_settings(){
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			if(
				isset($_POST['um_cap_settings_nonce']) &&
				wp_verify_nonce($_POST['um_cap_settings_nonce'], 'um_cap_settings')
			){
				if($_POST['um_cap_custom_field_prefix'] != $this->field_prefix){
					$new_prefix = rtrim(preg_replace("/[^a-zA-Z0-9-_.]/", "", $_POST['um_cap_custom_field_prefix']), "_") . "_";
					...
					if(!empty($test)){
						...
						$test = $this->wpdb->query("
							...
							WHERE meta_key LIKE 'um_cap_%'"
						);

						$test2 = $this->wpdb->query("
							UPDATE `{$this->db_prefix}um_cap_fields` cf
							SET slug = REPLACE(slug, '{$this->field_prefix}', '{$new_prefix}')
							WHERE slug LIKE 'um_cap_%'"
						);

						if($test !== false && $test2 !== false){
							$this->wpdb->query("COMMIT");
							update_option( 'um_cap_custom_field_prefix', $new_prefix );
						} else {
				...
				update_option('um_cap_data_to_delete', $_POST['um_cap_data_to_delete']);
	 ...
	 * @return bool
	 */
	private function um_cap_fields(){
		if($_SERVER['REQUEST_METHOD'] == 'POST'){
			switch($_POST['um_cap_action']){
				case 'um_cap_add_field':
					if(!$this->um_cap_add_edit_field()){
						$this->show_settings_header('');
						$this->show_add_field_form();
						$this->show_settings_footer();
						return false;
	 ...
	 * @return bool
	 */
	private function um_cap_add_edit_field(){
		if($_POST['um_cap_action'] == 'um_cap_edit_field'){
			$field_id = $_POST['field_id'];
			$um_cap_nonce_value = $_POST['um_cap_edit_field_nonce'];
			$um_cap_nonce_action = 'um_cap_edit_field' . "_" . $_POST['field_id'];
			$edit = true;
		} else {
			$edit = false;
			$um_cap_nonce_value = $_POST['um_cap_add_field_nonce'];
			$um_cap_nonce_action = 'um_cap_add_field';
		}

		if(!wp_verify_nonce($um_cap_nonce_value, $um_cap_nonce_action)){
			$this->add_notice(2, __("Looks like you didn't mean to do that."));
			return false;
		}

		$this->fetch_field_data();
		$field_slug = preg_replace("/[^0-9A-Za-z-_.]+/", "", $_POST['um_cap_field_slug']);
		$field_name = sanitize_text_field($_POST['um_cap_field_name']);
		$field_description = sanitize_text_field($_POST['um_cap_field_description']);
		...
		if($edit){
			$query = "UPDATE `{$this->db_prefix}um_cap_fields` SET `name` = '$field_name', `slug` = '$field_slug', `description` = '$field_description' WHERE id = $field_id;";
		} else {
			$query = "INSERT INTO `{$this->db_prefix}um_cap_fields` (`name`,`slug`,`description`) VALUES ('$field_name', '$field_slug', '$field_description');";
		}
	...
	public function delete_field(){
		...
		if(!isset($_GET['field_id']) || !isset($_GET['um_cap_delete_field_nonce'])){
		...
		if(!wp_verify_nonce($_GET['um_cap_delete_field_nonce'], 'um_cap_delete_field_' . $field_id)){
			echo "<p>" . __("Looks like you didn't mean to do that.") . "</p>";
			return false;
		}

		$this->wpdb->delete($this->db_prefix . "um_cap_fields", array('id'=>$field_id));

		wp_redirect(admin_url("admin.php?page=um_cap_fields"), 302);
	}
	...
	private function fetch_field_data($force = false){
		...
		$results = $this->wpdb->get_results("
			SELECT
				...
				cf.description,
				cf.date_created
			FROM `{$this->db_prefix}um_cap_fields` cf");
	...
	public function add_plugin_action_links($links){
		$settings_link = '<a href="admin.php?page=um_cap_settings">' . __( 'Settings' ) . '</a>';
	...
	?>
		<h2 class="nav-tab-wrapper">
			<a class="nav-tab <?php echo $main_settings_tab_active; ?>" id="um_cap_fields_tab" href="admin.php?page=um_cap_settings"><?php _e("Main Settings"); ?></a><a class="nav-tab <?php echo $fields_tab_active; ?>" id="um_cap_fields_tab" href="admin.php?page=um_cap_fields"><?php _e("Fields"); ?></a>
		</h2>
		<?php
	}
	...
	private function show_settings_page(){
		?>
		<form method="post" action="admin.php?page=um_cap_settings" novalidate="novalidate">
			<input type="hidden" name="um_cap_settings_nonce" value="<?php echo wp_create_nonce('um_cap_settings'); ?>" />
			<table class="form-table">
				<tr>
					<th>
						<label for="um_cap_data_to_delete"><?php _e("Data to Delete on Uninstall"); ?></label>
					</th>
					<td>
						<select id="um_cap_data_to_delete" name="um_cap_data_to_delete">
							<?php
							$options = array(
								1 => __("None"),
							...
							$current_option_val = get_option("um_cap_data_to_delete");
				...
				<tr>
					<th>
						<label for="um_cap_custom_field_prefix"><?php _e('Author Metadata Prefix'); ?></label>
					</th>
					<td>
						<input type="text" value="<?php echo get_option("um_cap_custom_field_prefix"); ?>" name="um_cap_custom_field_prefix" id="um_cap_custom_field_prefix" />
	...
	private function show_fields_page(){
			...
			<table class="wp-list-table widefat fixed striped">
				...
				foreach($this->field_list as $field){
					...
					<tr>
						...
						<td><a href="admin.php?page=um_cap_fields&func=edit_field&field_id=<?php echo $field->id; ?>"><?php echo $field->field_name; ?></a></td>
						<td><?php echo str_replace($this->field_prefix, '', $field->slug); ?></td>
						<td><?php echo $field->date_created; ?></td>
						<td style="text-align: right;">
							<a href="admin.php?page=um_cap_fields&func=edit_field&field_id=<?php echo $field->id; ?>"><?php _e("Edit"); ?></a> -
							<a class="delconfirm" data-delete-message="Type 'DELETE' to confirm you wish to delete the field '<?php echo $field->field_name; ?>'." href="admin.php?page=um_cap_fields&func=delete_field&field_id=<?php echo $field->id; echo '&um_cap_delete_field_nonce=' . wp_create_nonce("um_cap_delete_field_" . $field->id); ?>"><?php _e("Delete"); ?></a>
						</td>
					</tr>
				<?php } ?>
				</tbody>
		...
		?>
		<p class="submit"><a href="admin.php?page=um_cap_fields&func=add_new_field" class="button button-primary"><?php _e("Add New Field"); ?></a></p>
		<?php
	}
	...
	private function show_add_field_form(){
		...
		<form method="post" novalidate="novalidate" action="admin.php?page=um_cap_fields">
			<input type="hidden" name="um_cap_action" value="um_cap_add_field" />
			<input type="hidden" name="um_cap_add_field_nonce" value="<?php echo wp_create_nonce('um_cap_add_field'); ?>" />
			<table class="form-table">
				<tr>
					<th>
						<label for="um_cap_field_name"><?php _e("Field Name"); ?></label>
					</th>
					<td>
						<input class="regular-text" id="um_cap_field_name" type="text" value="<?php echo $_POST['um_cap_field_name']; ?>" name="um_cap_field_name" />
						<p class="description"><?php _e('The stylized name for your field.'); ?></p>
					</td>
				</tr>
				<tr>
					<th>
						<label for="um_cap_field_slug"><?php _e("Field Slug"); ?></label>
					</th>
					<td>
						<input class="regular-text" id="um_cap_field_slug" type="text" value="<?php echo $_POST['um_cap_field_slug']; ?>" name="um_cap_field_slug" />
				...
				<tr>
					<th>
						<label for="um_cap_field_description"><?php _e("Field Description"); ?></label>
					</th>
					<td>
						<textarea rows="5" style="max-width:500px;width:100%;" name="um_cap_field_description" id="um_cap_field_description"><?php echo $_POST['um_cap_field_description']; ?></textarea>
	...
	private function show_edit_field_form($field_data){
		$field_name = $field_data ? $field_data->field_name:$_POST['um_cap_field_name'];
		$field_id = $field_data ? $field_data->id : $_POST['field_id'];
		$form_action = "um_cap_edit_field";
		$form_nonce_action = $form_action . "_" . $field_id;
		$field_slug = $field_data ? str_replace($this->field_prefix, '', $field_data->slug) : $_POST['um_cap_field_slug'];
		$field_description = $field_data ? $field_data->description : $_POST['um_cap_field_description'];
		
		?>
		<h3><?php echo __("Edit the field") . " '$field_name'"; ?></h3>
		<form method="post" novalidate="novalidate" action="admin.php?page=um_cap_fields">
			<input type="hidden" value="<?php echo $field_id; ?>" name="field_id" />
			<input type="hidden" name="um_cap_action" value="<?php echo $form_action; ?>" />
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<input class="regular-text" id="um_cap_field_name" type="text" value="<?php echo $field_name; ?>" name="um_cap_field_name" />
						<p class="description"><?php _e('The stylized name for your field.'); ?></p>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input class="regular-text" id="um_cap_field_slug" type="text" value="<?php echo $field_slug; ?>" name="um_cap_field_slug" />
						<p class="description"><?php _e('The slug used for your field (e.g.: twitter-username). This will always be prepended by the custom field prefix. Available characters: a-z A-Z 0-9 _ - .'); ?></p>
					</td>
				</tr>
				<tr>
					...
					<td>
						<textarea rows="5" style="max-width:500px;width:100%;" name="um_cap_field_description" id="um_cap_field_description"><?php echo $field_description; ?></textarea>
						<p class="description"><?php _e('Explain what this field is for.'); ?></p>
					</td>
				</tr>
			</table>
	...
	public function plugin_activation(){
		if(!empty($this->wpdb->get_results("SHOW TABLES LIKE '{$this->db_prefix}um_cap_fields'")))
			return true;

		if(get_option("um_cap_custom_field_prefix") === false)
			add_option("um_cap_custom_field_prefix", "um_cap_");

		if(get_option("um_cap_data_to_delete") === false)
			add_option("um_cap_data_to_delete", "1");

		require_once( ABSPATH . 'wp-admin/includes/upgrade.php' );

		dbDelta("CREATE TABLE `{$this->db_prefix}um_cap_fields` (
